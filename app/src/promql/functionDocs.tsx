...
const funcDocs: Record<string, React.ReactNode> = {
  abs: (
    <>
      <p>
        <code>abs(v instant-vector)</code> returns a vector containing all float samples in the input vector converted to
        their absolute value. Histogram samples in the input vector are ignored silently.
      </p>
    </>
  ),
  absent: (
    <>
      <p>
        ...
        samples or histogram samples) and a 1-element vector with the value 1 if the vector passed to it has no elements.
      </p>
  ...
  absent_over_time: (
    <>
      <p>
        ...
        elements (float samples or histogram samples) and a 1-element vector with the value 1 if the range vector passed to
        it has no elements.
      </p>
  ...
  acos: (
    <>
      ...
      <ul>
        ...
        <li>
          <code>asinh(v instant-vector)</code>: calculates the inverse hyperbolic sine of all float samples in <code>v</code>{' '}
  ...
  avg_over_time: (
    <>
      ...
      <ul>
        ...
        <li>
          <code>sum_over_time(range-vector)</code>: the sum of all float or histogram samples in the specified interval (see
          details below).
        </li>
        <li>
      ...
      <ul>
        ...
        <li>
          <code>ts_of_min_over_time(range-vector)</code>: the timestamp of the last float sample that has the minimum value
          of all float samples in the specified interval.
        </li>
        <li>
          <code>ts_of_max_over_time(range-vector)</code>: the timestamp of the last float sample that has the maximum value
          of all float samples in the specified interval.
        </li>
        <li>
      ...
      <ul>
        ...
        <li>
          <code>avg_over_time()</code> and <code>sum_over_time()</code> act on histogram samples in a way that corresponds to
          the respective aggregation operators. If a series contains a mix of float samples and histogram samples within the
          range, the corresponding result is removed entirely from the output vector. Such a removal is flagged by a
          warn-level annotation.
        </li>
        <li>
          All other functions ignore histogram samples in the following way: Input ranges containing only histogram samples
          are silently removed from the output. For ranges with a mix of histogram and float samples, only the float samples
          are processed and the omission of the histogram samples is flagged by an info-level annotation.
        </li>
      </ul>

      <p>
        <code>first_over_time(m[1m])</code> differs from <code>m offset 1m</code> in that the former will select the first
        sample of <code>m</code> <em>within</em> the 1m range, where <code>m offset 1m</code> will select the most recent
        sample within the lookback interval <em>outside and prior to</em> the 1m offset. This is particularly useful with{' '}
        <code>first_over_time(m[step()])</code>
  ...
  ceil: (
    <>
      <p>
        <code>ceil(v instant-vector)</code> returns a vector containing all float samples in the input vector rounded up to
        the nearest integer value greater than or equal to their original value. Histogram samples in the input vector are
        ignored silently.
      </p>
  ...
  changes: (
    <>
      <p>
        ...
        within the provided time range as an instant vector. A float sample followed by a histogram sample, or vice versa,
        counts as a change. A counter histogram sample followed by a gauge histogram sample with otherwise exactly the same
        values, or vice versa, does not count as a change.
      </p>
    </>
  ),
  clamp: (
    <>
      <p>
        <code>clamp(v instant-vector, min scalar, max scalar)</code> clamps the values of all float samples in <code>v</code>{' '}
        to have a lower limit of <code>min</code> and an upper limit of
  ...
  clamp_max: (
    <>
      <p>
        <code>clamp_max(v instant-vector, max scalar)</code> clamps the values of all float samples in <code>v</code> to have
        an upper limit of <code>max</code>. Histogram samples in the input vector are ignored silently.
      </p>
    </>
  ),
  clamp_min: (
    <>
      <p>
        <code>clamp_min(v instant-vector, min scalar)</code> clamps the values of all float samples in <code>v</code> to have
        a lower limit of <code>min</code>. Histogram samples in the input vector are ignored silently.
      </p>
    </>
  ),
  cos: (
  ...
  day_of_month: (
    <>
      <p>
        ...
        <code>v</code> as timestamps (number of seconds since January 1, 1970 UTC) and returns the day of the month (in UTC)
        for each of those timestamps. Returned values are from 1 to 31. Histogram samples in the input vector are ignored
        silently.
      </p>
    </>
  ),
  day_of_week: (
    <>
      <p>
        ...
        those timestamps. Returned values are from 0 to 6, where 0 means Sunday etc. Histogram samples in the input vector
  ...
  day_of_year: (
    <>
      <p>
        ...
        those timestamps. Returned values are from 1 to 365 for non-leap years, and 1 to 366 in leap years. Histogram samples
        in the input vector are ignored silently.
      </p>
    </>
  ),
  days_in_month: (
    <>
      <p>
        ...
        month of each of those timestamps (in UTC). Returned values are from 28 to 31. Histogram samples in the input vector
        are ignored silently.
      </p>
    </>
  ),
  ...
  delta: (
    <>
      ...
      <p>
        <code>delta</code> acts on histogram samples by calculating a new histogram where each component (sum and count of
        observations, buckets) is the difference between the respective component in the first and last native histogram in{' '}
        <code>v</code>. However, each element in <code>v</code> that contains a mix of float samples and histogram samples
        within the range will be omitted from the result vector, flagged by a warn-level annotation.
      </p>
  ...
  deriv: (
    <>
      <p>
        <code>deriv(v range-vector)</code> calculates the per-second derivative of each float time series in the range vector{' '}
        <code>v</code>, using <a href="https://en.wikipedia.org/wiki/Simple_linear_regression">simple linear regression</a>.
        The range vector must have at least two float samples in order to perform the calculation. When <code>+Inf</code> or{' '}
        <code>-Inf</code> are found in the range vector, the slope and offset value calculated will be <code>NaN</code>.
      </p>

      <p>
        <code>deriv</code> should only be used with gauges and only works for float samples. Elements in the range vector
        that contain only histogram samples are ignored entirely. For elements that contain a mix of float and histogram
  ...
  double_exponential_smoothing: (
    <>
      ...
      <p>
        <code>double_exponential_smoothing(v range-vector, sf scalar, tf scalar)</code> produces a smoothed value for each
        float time series in the range in <code>v</code>. The lower the smoothing factor <code>sf</code>, the more importance
        is given to old data. The higher the trend factor <code>tf</code>, the more trends in the data is considered. Both{' '}
        <code>sf</code> and
        <code>tf</code> must be between 0 and 1. For additional details, refer to{' '}
        <a href="https://www.itl.nist.gov/div898/handbook/pmc/section4/pmc433.htm">NIST Engineering Statistics Handbook</a>.
        In Prometheus V2 this function was called <code>holt_winters</code>. This caused confusion since the Holt-Winters
        method usually refers to triple exponential smoothing. Double exponential smoothing as implemented here is also
        referred to as &ldquo;Holt Linear&rdquo;.
      </p>

      <p>
        <code>double_exponential_smoothing</code> should only be used with gauges and only works for float samples. Elements
        in the range vector that contain only histogram samples are ignored entirely. For elements that contain a mix of
        float and histogram samples, only the float samples are used as input, which is flagged by an info-level annotation.
      </p>
    </>
  ),
  end: (
    <>
      ...
      <p>
        <code>end()</code> returns the end timestamp of the current query range evaluation as the number of seconds since
        January 1, 1970 UTC. For instant queries, this is equal to the evaluation timestamp.
      </p>
    </>
  ),
  exp: (
  ...
  floor: (
    <>
      <p>
        <code>floor(v instant-vector)</code> returns a vector containing all float samples in the input vector rounded down
        to the nearest integer value smaller than or equal to their original value. Histogram samples in the input vector are
        ignored silently.
      </p>
  ...
  histogram_avg: (
    <>
      <p>
        <code>histogram_avg(v instant-vector)</code> returns the arithmetic average of observed values stored in each native
  ...
  histogram_count: (
    <>
      ...
      <p>
        ...
        to “requests per second”) from a series of histogram samples:
      </p>
  ...
  histogram_fraction: (
    <>
      <p>
        ...
        <code>b</code>. Float samples in <code>b</code> are considered the counts of observations in each bucket of one or
        more classic histograms, while native histogram samples in <code>b</code> are treated each individually as a separate
        histogram. This works in the same way as for <code>histogram_quantile()</code>. (See there for more details.)
      </p>
      ...
      <p>
        ...
        <code>histogram_quantile()</code>. (See there for more details.) Especially with classic histograms, it is easy to
        accidentally pick lower or upper values that are very far away from any bucket boundary, leading to large margins of
        error. Rather than using <code>histogram_fraction()</code> with classic histograms, it is often a more robust
        approach to directly act on the bucket series when calculating fractions. See the
      ...
      <p>
        ...
        definition of the histogram. (The usual standard exponential schemas all feature inclusive upper boundaries and
        exclusive lower boundaries for positive values, and vice versa for negative values.) Without a precise alignment of
        boundaries, the function uses interpolation to estimate the fraction. With the resulting uncertainty, it becomes
      ...
      <p>
        ...
        <code>histogram_fraction(-Inf, +Inf, b)</code> effectively returns the fraction of non-<code>NaN</code> observations
        and may therefore be less than 1.
      </p>
    </>
  ),
  histogram_quantile: (
    <>
      ...
      <p>
        The (native) histogram samples in <code>b</code> are treated each individually as a separate histogram to calculate
        the quantile from.
      </p>
      ...
      <p>
        ...
        <code>histogram_quantile()</code> function interpolates the quantile value within the bucket the quantile value falls
        into. For classic histograms, for native histograms with custom bucket boundaries, and for the zero bucket of other
        native histograms, it assumes a uniform distribution of observations within the bucket (also called{' '}
        <em>linear interpolation</em>). For the non-zero-buckets of native histograms with a standard exponential bucketing
        schema, the interpolation is done under the assumption that the samples within the bucket are distributed in a way
        that they would uniformly populate the buckets in a hypothetical histogram with higher resolution. (This is also
        called <em>exponential interpolation</em>. See the{' '}
      ...
      <ul>
        ...
        <li>If a quantile is located in the highest bucket, the upper bound of the second highest bucket is returned.</li>
        <li>
          The lower limit of the lowest bucket is assumed to be 0 if the upper bound of that bucket is greater than 0. In
          that case, the usual linear interpolation is applied within that bucket. Otherwise, the upper bound of the lowest
      ...
      <ul>
        ...
        <li>
          If a native histogram with standard exponential buckets has <code>NaN</code>
          observations and the quantile falls above all of the existing exponential buckets, <code>NaN</code> is returned.
          This is flagged with an info level annotation.
        </li>
        <li>
          A zero bucket with finite width is assumed to contain no negative observations if the histogram has observations in
          positive buckets, but none in negative buckets.
        </li>
        <li>
          A zero bucket with finite width is assumed to contain no positive observations if the histogram has observations in
          negative buckets, but none in positive buckets.
        </li>
      </ul>
      ...
      <p>
        ...
        latter is evidence for an actual issue with the input data and is therefore flagged by an info-level annotation
  ...
  histogram_quantiles: (
    <>
      ...
      <p>
        <code>histogram_quantiles(v instant-vector, quantile_label string, φ_1 scalar, φ_2 scalar, ...)</code> calculates
        multiple (between 1 and 10) φ-quantiles (0 ≤ φ ≤ 1) from a{' '}
        <a href="https://prometheus.io/docs/concepts/metric_types/#histogram">classic histogram</a> or from a native
        histogram. Quantile calculation works the same way as in <code>histogram_quantile()</code>. The second argument (a
        string) specifies the label name that is used to identify different quantiles in the query result.
      </p>
  ...
  histogram_stddev: (
    <>
      <p>
        <code>histogram_stddev(v instant-vector)</code> returns the estimated standard deviation of observations for each
        native histogram sample in <code>v</code>. For this estimation, all observations in a bucket are assumed to have the
        value of the mean of the bucket boundaries. For the zero bucket and for buckets with custom boundaries, the
        arithmetic mean is used. For the usual exponential buckets, the geometric mean is used. Float samples are ignored and
        do not show up in the returned vector.
      </p>

      <p>
        Similarly, <code>histogram_stdvar(v instant-vector)</code> returns the estimated variance of observations for each
        native histogram sample in <code>v</code>.
      </p>
    </>
  ),
  histogram_stdvar: (
  ...
  hour: (
    <>
      <p>
        <code>hour(v=vector(time()) instant-vector)</code> interprets float samples in <code>v</code> as timestamps (number
        of seconds since January 1, 1970 UTC) and returns the hour of the day (in UTC) for each of those timestamps. Returned
        values are from 0 to 23. Histogram samples in the input vector are ignored silently.
      </p>
    </>
  ),
  idelta: (
    <>
      <p>
        ...
        <code>v</code>, returning an instant vector with the given deltas and equivalent labels. Both samples must be either
        float samples or histogram samples. Elements in <code>v</code> where one of the last two samples is a float sample
        and the other is a histogram sample will be omitted from the result vector, flagged by a warn-level annotation.
      </p>
  ...
  increase: (
    <>
      ...
      <p>
        <code>increase</code> acts on histogram samples by calculating a new histogram where each component (sum and count of
      ...
      <p>
        <code>increase</code> should only be used with counters (for both floats and histograms). It is syntactic sugar for{' '}
        <code>rate(v)</code> multiplied by the number of seconds under the specified time range window, and should be used
        primarily for human readability. Use <code>rate</code> in recording rules so that increases are tracked consistently
  ...
  info: (
    <>
      ...
      <p>
        ...
        <code>v</code>, all info series with matching <em>identifying</em> labels (more on this later), and adds the union of
        their <em>data</em> (i.e., non-identifying) labels to the time series. The second argument{' '}
        <code>data-label-selector</code> is optional. It is not a real instant vector, but uses a subset of its syntax. It
        must start and end with curly braces (
        <code>
          {'{'} ... {'}'}
        </code>
        ) and may only contain label matchers. The label matchers are used to constrain which info series to consider and
        which data labels to add to <code>v</code>.
      </p>

      <p>
        If there is no matching info series for a given time series in <code>v</code> at a particular timestamp (e.g. because
        the info series has gone stale), the behavior depends on the data label matchers: If the{' '}
      ...
      <p>
        Identifying labels of an info series are the subset of labels that uniquely identify the info series. The remaining
        labels are considered
        <em>data labels</em> (also called non-identifying). (Note that Prometheus&rsquo;s concept of time series identity
        always includes <em>all</em> the labels. For the sake of the <code>info</code>
        function, we “logically” define info series identity in a different way than in the conventional Prometheus view.)
        The identifying labels of an info series are used to join it to regular (non-info) series, i.e. those series that
        have the same labels as the identifying labels of the info series. The data labels, which are the ones added to the
        regular series by the <code>info</code> function, effectively encode metadata key value pairs. (This implies that a
        change in the data labels in the conventional Prometheus view constitutes the end of one info series and the
        beginning of a new info series, while the “logical” view of the <code>info</code> function is that the same info
        series continues to exist, just with different “data”.)
      </p>

      <p>
        The conventional approach of adding data labels is sometimes called a “join query”, as illustrated by the following
        example:
      </p>
      ...
      <p>
        The core of the query is the expression <code>rate(http_server_request_duration_seconds_count[2m])</code>. But to add
        data labels from an info metric, the user has to use elaborate (and not very obvious) syntax to specify which info
        metric to use (<code>target_info</code>), what the identifying labels are (<code>on (job, instance)</code>), and
        which data labels to add (<code>group_left (k8s_cluster_name)</code>).
      </p>

      <p>
        This query is not only verbose and hard to write, it might also run into an “identity crisis”: If any of the data
        labels of <code>target_info</code> changes, Prometheus sees that as a change of series (as alluded to above,
        Prometheus just has no native concept of non-identifying labels). If the old <code>target_info</code> series is not
        properly marked as stale (which can happen with certain ingestion paths), the query above will fail for up to 5m (the
        lookback delta) because it will find a conflicting match with both the old and the new version of{' '}
        <code>target_info</code>.
      </p>

      <p>
        The <code>info</code> function not only resolves this conflict in favor of the newer series, it also simplifies the
        syntax because it knows about the available info series and what their identifying labels are. The example query
        looks like this with the <code>info</code> function:
      </p>
      ...
      <p>
        While <code>info</code> normally automatically finds all matching info series, it&rsquo;s possible to restrict them
        by providing a <code>__name__</code> label matcher, e.g.
        <code>
      ...
      <p>
        ...
        default <code>target_info</code> if that argument is not specified), they will be treated as info series and will be
        returned unchanged.
      </p>
      ...
      <p>
        ...
        <code>instance</code> and <code>job</code>. <code>info</code> does support other info series names however, through
      ...
      <p>
        ...
        <code>.+_info</code> and then applies the negated matchers as filters. This is because negated matchers alone cannot
        positively identify which info metrics to consider.
      </p>

      <p>
        These limitations are partially defeating the purpose of the <code>info</code> function. At the current stage, this
        is an experiment to find out how useful the approach turns out to be in practice. A final version of the{' '}
  ...
  irate: (
    <>
      <p>
        ...
        restarts) are automatically adjusted for. Both samples must be either float samples or histogram samples. Elements in{' '}
        <code>v</code> where one of the last two samples is a float sample and the other is a histogram sample will be
        omitted from the result vector, flagged by a warn-level annotation.
      </p>
      ...
      <p>
        ...
        aggregating over time (any function ending in <code>_over_time</code>), always take an <code>irate()</code> first,
  ...
  label_replace: (
    <>
      <p>
        ...
        matches the <a href="./basics.md#regular-expressions">regular expression</a> <code>regex</code> against the value of
        the label <code>src_label</code>. If it matches, the value of the label <code>dst_label</code> in the returned
        timeseries will be the expansion of <code>replacement</code>, together with the original labels in the input.
        Capturing groups in the regular expression can be referenced with <code>$1</code>, <code>$2</code>, etc. Named
  ...
  ln: (
    <>
      <p>
        <code>ln(v instant-vector)</code> calculates the natural logarithm for all float samples in <code>v</code>. Histogram
        samples in the input vector are ignored silently. Special cases are:
      </p>
  ...
  minute: (
    <>
      <p>
        <code>minute(v=vector(time()) instant-vector)</code> interprets float samples in <code>v</code> as timestamps (number
        of seconds since January 1, 1970 UTC) and returns the minute of the hour (in UTC) for each of those timestamps.
        Returned values are from 0 to 59. Histogram samples in the input vector are ignored silently.
      </p>
    </>
  ),
  month: (
    <>
      <p>
        <code>month(v=vector(time()) instant-vector)</code> interprets float samples in <code>v</code> as timestamps (number
        of seconds since January 1, 1970 UTC) and returns the month of the year (in UTC) for each of those timestamps.
        Returned values are from 1 to 12, where 1 means January etc. Histogram samples in the input vector are ignored
        silently.
      </p>
    </>
  ),
  ...
  predict_linear: (
    <>
      <p>
        ...
        have at least two float samples in order to perform the calculation. When <code>+Inf</code> or <code>-Inf</code> are
        found in the range vector, the predicted value will be <code>NaN</code>.
      </p>

      <p>
        <code>predict_linear</code> should only be used with gauges and only works for float samples. Elements in the range
  ...
  range: (
    <>
      ...
      <p>
        <code>range()</code> returns the range duration of the current query range evaluation in seconds and is equivalent to{' '}
        <code>end() - start()</code>. For instant queries, this returns <code>0</code>.
      </p>
    </>
  ),
  rate: (
    <>
      ...
      <p>
        The following example expression returns the per-second average rate of HTTP requests over the last 5 minutes, per
      ...
      <p>
        <code>rate</code> should only be used with counters (for both floats and histograms). It is best suited for alerting,
  ...
  resets: (
    <>
      <p>
        ...
        disappearance of any previously populated bucket, a decrease of the zero-bucket width, or any schema change that is
        not a compatible decrease of resolution.
      </p>
      ...
      <p>
        A float sample followed by a histogram sample, or vice versa, counts as a reset. A counter histogram sample followed
        by a gauge histogram sample, or vice versa, also counts as a reset (but note that <code>resets</code> should not be
        used on gauges in the first place, see above).
      </p>
    </>
  ),
  round: (
  ...
  scalar: (
    <>
      <p>
        ...
        <code>scalar(v instant-vector)</code> returns the sample value of that float sample as a scalar. If the input vector
        does not have exactly one element with a float sample, <code>scalar</code> will return <code>NaN</code>. Histogram
  ...
  sgn: (
    <>
      <p>
        <code>sgn(v instant-vector)</code> returns a vector with all float sample values converted to their sign, defined as
        this: 1 if v is positive, -1 if v is negative and 0 if v is equal to zero. Histogram samples in the input vector are
        ignored silently.
      </p>
    </>
  ),
  ...
  sort: (
    <>
      <p>
        <code>sort(v instant-vector)</code> returns vector elements sorted by their float sample values, in ascending order.
        Histogram samples in the input vector are ignored silently.
      </p>
  ...
  sort_by_label: (
    <>
      ...
      <p>
        <code>sort_by_label</code> uses <a href="https://en.wikipedia.org/wiki/Natural_sort_order">natural sort order</a>.
      </p>
    </>
  ),
  sort_by_label_desc: (
  ...
  start: (
    <>
      ...
      <p>
        <code>start()</code> returns the start timestamp of the current query range evaluation as the number of seconds since
        January 1, 1970 UTC. For instant queries, this is equal to the evaluation timestamp.
      </p>
    </>
  ),
  ...
  step: (
    <>
      ...
      <p>
        <code>step()</code> returns the query resolution step as the number of seconds. For instant queries, this returns{' '}
        <code>0</code>.
      </p>
    </>
  ),
  ...
  vector: (
    <>
      <p>
        <code>vector(s scalar)</code> converts the scalar <code>s</code> to a float sample and returns it as a single-element
        instant vector with no labels.
      </p>
    </>
  ),
  year: (
    <>
      <p>
        <code>year(v=vector(time()) instant-vector)</code> returns the year for each of the given times in UTC. Histogram
        samples in the input vector are ignored silently.
      </p>
    </>
  ),
