...
import { formatPrometheusFloat, parsePrometheusFloat } from '../utils/utils';
import { binaryOperatorType, vectorMatchCardinality, VectorMatching } from './ast';
...
export const resultMetric = (lhs: Metric, rhs: Metric, op: binaryOperatorType, matching: VectorMatching): Metric => {
  const result: Metric = {};
...
export const computeVectorVectorBinOp = (
  ...
  [lhs, rhs] = matching.card === vectorMatchCardinality.oneToMany ? [rhs, lhs] : [lhs, rhs];
  ...
  rhs.forEach((rs) => {
    ...
    if (!limits?.maxSeriesPerGroup || groups[sig].rhsCount < limits.maxSeriesPerGroup) {
  ...
  lhs.forEach((ls) => {
    ...
    if (!limits?.maxSeriesPerGroup || groups[sig].lhsCount < limits.maxSeriesPerGroup) {
    ...
    mg.rhs.forEach((rs) => {
      mg.lhs.forEach((ls, lIdx) => {
        ...
        mg.result.push({
          sample: {
            metric: metric,
            value: [ls.value[0], keep || bool ? formatPrometheusFloat(value) : filteredSampleValue],
          },
          manySideIdx: lIdx,
        });
      });
  ...
  if (matching.card === vectorMatchCardinality.oneToMany) {
    Object.keys(groups).forEach((sig) => {
      ...
      [groups[sig].lhsCount, groups[sig].rhsCount] = [groups[sig].rhsCount, groups[sig].lhsCount];
    });
  }
